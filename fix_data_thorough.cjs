...
code = code.replace(/let s = epoch \* \d+ \+ \d+;/, 'let s = epoch * 1234 + 5678;');

// Update baseOrderData using a smarter replacement
const newProducts = [
...
let pIndex = 0;
// We will replace block by block
code = code.replace(/product:\s*"[^"]+",\s*productValue:\s*\d+,\s*payout:\s*\d+,\s*avatar:\s*"[^"]+",\s*productImage:\s*"[^"]+"/g, (match) => {
  const p = newProducts[pIndex % newProducts.length];
  pIndex++;
  
  // Extract avatar to keep it
  const avatarMatch = match.match(/avatar:\s*"([^"]+)"/);
  const avatar = avatarMatch ? avatarMatch[1] : '';

  const payout = p.v * 0.05;
...
// Update comments generator to ensure 40+ comments
// First, replace commentTemplates with a larger array
const commentsStr = \`const commentTemplates = [
  { q: (m, p) => \\\`Kiukweli huu mtandao umenisaidia sana! Nimeverify order ya \\\${p} asubuhi hii na tayari wamenitumia kamisheni yangu moja kwa moja. Siwezi kuacha kufanya hii kazi. 🔥💸\\\`, tag: "Malipo Yamepokelewa 💰", reply: null },
  ...
  { q: (m) => \\\`Kama bado unalaza damu unakosa pesa za bure! Mtaji wa 14,500 niliouweka juzi, leo nisharudisha mara mbili yake. M-Pesa inasoma vizuri mchana huu. Hii sio ya kukosa! 🚀💪\\\`, tag: "Mtaji Umerudi Haraka ⚡", reply: null },
  ...
  // Adding more varied ones to make it easy to generate 40+
...
if (cStart !== -1 && cEnd !== -1) {
  code = code.substring(0, cStart) + commentsStr + code.substring(cEnd + 5);
}

// Modify generate6HourComments logic to generate ~45 comments
code = code.replace(
  'const commentMembers = shuffled.slice(0, 10 + Math.floor(rnd() * 8)); // 10 to 17 comments',
  'const commentMembers = shuffled.slice(0, 42 + Math.floor(rnd() * 8)); // 42 to 49 comments'
);

fs.writeFileSync('src/data.ts', code);
console.log('Fixed data thoroughly.');
